...
def HIBT_scrap(img_path):
    ...
    try:
        ...
        while True:
            ...
            ## 추출 개수(index)값이 더 큰 경우가 있음??? == 같은 이미지 2번 추출
            if img_cnt == index:
                break

            try:
                while index < img_cnt:
                    ## click() X ==> send_keys(Keys.ENTER)
                    images[index].send_keys(Keys.ENTER)
                ...
                print(e) ## element click intercepted: Element = 이미지 클릭할 수 없음
                ## continue
                try:
                    while index < img_cnt:
                        ...
                        ## //*[@id='root']/div/main/div/div[3]/div[0]/img == [0]이 없음
                        ## 이미지를 클릭할 수 없을때 == driver.execute_script("arguments[0].click();", elements)
                        elements = driver.find_element(By.XPATH, 
                                                       f"//*[@id='root']/div/main/div/div[3]/div[{index+1}]/img")
                        driver.execute_script("arguments[0].click();", elements)
                        img_url = elements.get_attribute('src')
